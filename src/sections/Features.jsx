      ...
      <Element name="features">
        <div className={`container`}>
          ...
          >
            ...
            <ul
              className={`relative flex justify-around flex-row px-[5%] border-2 border-s3 rounded-7xl max-md:hidden w-full`}
            >
              <div
                className={`absolute bg-s3/20 top-[38%] left-0 right-0 w-full h-[1px] z-10`}
              />
              {details.map(({ id, icon, title }) => (
                <li key={id} className={`relative pt-16 px-4 pb-14`}>
                  <div
                    className={`absolute top-0 bottom-0 left-1/2 bg-s3/20 h-full z-10`}
                  />
